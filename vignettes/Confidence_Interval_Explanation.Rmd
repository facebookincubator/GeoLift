...
However, because we use the absolute value of the error of the model in each one of the moved dates, the confidence interval estimated by this method tends to be larger than the true credible interval of the estimated treatment effect.
...
This happens almost always because we use `conformal_type` as `"iid"`, which assumes that the values observed at each date are independent of the values from all other dates (and identically distributed). This assumption is usually false for most use-cases, as the values are usually weakly connected to each other. The consequence of this wrong assumption is that the estimated p-value for basically any possible ATT value becomes close to 0, meaning that we can really differentiate the likelihood of one lift value to the other. This is usually solved by changing the `conformal_type` input to `"block"`.


**Sometimes I get a Confidence Interval for my lift in a given date that is negative, even though the p-value for the Average Treatment Effect (ATT) is very lowe (e.g. 1%)**
