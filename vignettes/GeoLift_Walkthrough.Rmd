...
knit:
  rmarkdown::render("GeoLift_Walkthrough.Rmd", output_format = "all")
---
...
The results in `MarketSelection` show that the test markets with the best ranks are: `(chicago, cincinnati, houston, portland)`, `(atlanta, chicago, cleveland, las vegas, reno)` and `(chicago, portland)`, all of them tied at rank 1. We can `plot()` these last two  results to inspect them further. This plot will show how the results of the `GeoLift()` model would look like with the latest possible test period as well as the test's power curve across all simulations.
